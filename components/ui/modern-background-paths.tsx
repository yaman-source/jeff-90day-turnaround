...
function SpiralPaths() {
  const spirals = Array.from({ length: 8 }, (_, i) => {
    ...
    for (let angle = 0; angle <= turns * 360; angle += 5) {
      ...
      path += ` L${centerX + r * Math.cos(rad)},${centerY + r * Math.sin(rad)}`;
    }
    ...
    <div className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
      ...
      <div className="relative z-10 container mx-auto px-4 md:px-6 text-center">
        ...
        >
          ...
          >
            <div className="relative p-[2px] bg-gradient-to-r from-orange-500 via-amber-500 to-yellow-500 rounded-2xl group-hover:from-orange-600 group-hover:via-amber-600 group-hover:to-yellow-600 transition-all duration-300">
              <Button
                variant="ghost"
                size="lg"
                className="relative rounded-[14px] px-12 py-6 text-lg font-semibold
                  ...
                  group-hover:-translate-y-1 group-hover:shadow-2xl border-0 backdrop-blur-sm"
              >
                <motion.span
      ...
      {/* Floating elements */}
      <motion.div
        className="absolute top-1/4 left-1/4 w-4 h-4 bg-orange-500/20 rounded-full blur-sm"
        animate={{ y: [0, -20, 0], x: [0, 10, 0], scale: [1, 1.2, 1], opacity: [0.3, 0.7, 0.3] }}
        transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute top-3/4 right-1/3 w-6 h-6 bg-amber-500/20 rounded-full blur-sm"
